...
function resolveTarget(args: {
  ...
  if (hasId && hasRef) return { error: 'Pass either item_id or ref, not both.' }
  if (!hasId && !hasRef) {
    return { error: 'Pass item_id for an existing item, or ref for one proposed in this run.' }
  }
  return { itemId: hasId ? args.item_id! : null, ref: hasRef ? args.ref! : null }
}

/* ── digests: compact, agent-facing projections of domain objects ────────── */

function messageDigest(m: Awaited<ReturnType<AgentRepo['listRunMessages']>>[number]): unknown {
  return {
    message_id: m.id,
...
function itemDigest(i: Awaited<ReturnType<AgentRepo['listItems']>>[number]): unknown {
  return {
    item_id: i.id,
    company: i.company,
...
export function createMcpServer(deps: McpDeps): McpBridge {
  ...
  async function traced(
    ...
    emit({ runId, tool, phase: 'start', detail, at: new Date().toISOString() })
    try {
      ...
      emit({ runId, tool, phase: 'error', detail, error: message, at: new Date().toISOString() })
      return fail(message)
    }
  }
  ...
  function readProposalId(result: ToolResult): number | undefined {
    ...
    try {
      const parsed = JSON.parse(result.content[0]?.text ?? '{}') as { proposal_id?: number }
      return typeof parsed.proposal_id === 'number' ? parsed.proposal_id : undefined
    } catch {
      return undefined
    }
  }
  ...
  function buildServer(runId: number): McpServer {
    ...
    server.registerTool(
      ...
      async (args: GetMessageArgs) =>
        traced(runId, 'get_message', `message ${args.message_id}`, async () => {
    ...
    server.registerTool(
      ...
      async (args: ListItemsArgs) =>
        traced(runId, 'list_items', args.query ?? args.status ?? null, async () => {
    ...
    server.registerTool(
      'get_item',
      {
        ...
        description: 'One application plus its full timeline and the messages linked to it.',
    ...
    server.registerTool(
      ...
      async (args: ProposeUpdateItemArgs) =>
        traced(runId, 'propose_update_item', `item ${args.item_id}`, async () => {
          if (!(await repo.getItem(args.item_id))) return fail(`Item ${args.item_id} not found.`)
          const fields = args.fields as ItemFieldPatch
          if (Object.keys(fields).length === 0) return fail('`fields` is empty — nothing to change.')
          const payload: UpdateItemProposalPayload = { item_id: args.item_id, fields }
      ...
      async (args: ProposeAddEventArgs) =>
        traced(runId, 'propose_add_event', args.title, async () => {
          ...
          if (
            ...
            return fail(`Message ${args.message_id} is not in this run’s allowlist.`)
          }
    ...
    server.registerTool(
      ...
      async (args: ProposeLinkMessageArgs) =>
        traced(runId, 'propose_link_message', `message ${args.message_id}`, async () => {
          const target = resolveTarget(args)
          if ('error' in target) return fail(target.error)
          if (!(await repo.isMessageAllowed(runId, args.message_id))) {
            return fail(`Message ${args.message_id} is not in this run’s allowlist.`)
  ...
  async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (sessionId) {
      ...
      if (!session || session.token !== token) return send(res, 404, -32001, 'Session not found')
      const body = req.method === 'POST' ? await readBody(req) : undefined
      await session.transport.handleRequest(req, res, body)
      return
    }

    if (req.method !== 'POST') return send(res, 400, -32000, 'Missing mcp-session-id')
  ...
  return {
      ...
      starting = new Promise<number>((resolve, reject) => {
        const server = createServer((req, res) => {
          handle(req, res).catch((err: unknown) => {
            ...
            send(res, 400, -32000, err instanceof Error ? err.message : 'Bad request')
          })
        })
    ...
    mcpConfigJson(token: string): string {
      if (port == null) throw new Error('MCP server is not started — call start() first.')
